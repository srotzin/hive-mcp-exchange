...
const _bogoDB = new _BogoDatabase(process.env.BOGO_DB_PATH || '/tmp/bogo_exchange.db');
...
// ─── POST /v1/exchange/quote — pay-front for requesting a market quote ───
// Returns 402 + BOGO block with no tx_hash. First-call-free for new DIDs.
// On payment: returns a stub quote envelope (live pricing from HiveExchange).
app.post('/v1/exchange/quote', async (req, res) => {
  const PRICE = 0.001;
